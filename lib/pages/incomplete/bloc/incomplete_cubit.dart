...
import 'incomplete_state.dart';

@Injectable()
class IncompleteCubit extends Cubit<IncompleteState> with EventBusMixin {
  IncompleteCubit(this._taskRepository) : super(const IncompleteState()) {
  ...
  Future<void> _getTasks() async {
    ...
    try {
      ...
      if (result != null) {
        final tasks =
        result.where((element) => element.status == TaskStatus.incomplete).toList();
  ...
  Future<void> _fetchTasks() async {
    ...
    try {
      ...
      if (result is DataSuccess) {
        final tasks = result.data?.where((element) => element.status == TaskStatus.incomplete).toList();
        emit(state.copyWith(
            task: tasks,
            dataStatus: (tasks?.isNotEmpty ?? false)
